...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            {showPanel && renderPanel ? <div className={`absolute left-1/2 top-full z-[70] ${data.type === CanvasNodeType.Image ? "w-[600px]" : "w-[500px]"} -translate-x-1/2 pt-4`}>{renderPanel(data)}</div> : null}
        </div>
    );
});

function NodeContent(props: NodeContentRendererProps) {
    if ((props.node.type === CanvasNodeType.Config || props.node.type === CanvasNodeType.ReferenceSet || props.node.type === CanvasNodeType.Generation || props.node.type === CanvasNodeType.ResultGroup) && props.renderNodeContent) return props.renderNodeContent(props.node);
